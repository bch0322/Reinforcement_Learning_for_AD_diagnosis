...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=1, stride=1, dilation=1):
        super(Bottleneck, self).__init__()
        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=1, bias=False)
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=kernel_size, stride=stride, dilation=dilation, padding=dilation-1, bias=False)
        self.conv3 = nn.Conv3d(planes, planes * self.expansion, kernel_size=1, bias=False)

        self.norm1 = nn.InstanceNorm3d(planes, affine=True)
        self.norm2 = nn.InstanceNorm3d(planes, affine=True)
        self.norm3 = nn.InstanceNorm3d(planes * self.expansion, affine=True)

        self.act_f = nn.ReLU(inplace=True)

        if stride != 1 or inplanes != planes * self.expansion or kernel_size != 1:
    ...
    def forward(self, x, **kwargs):
        ...
        residual = x[:, :, self.kernel_size // 2:, self.kernel_size // 2:, self.kernel_size // 2:]
...
class Bottleneck_wo_norm(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, kernel_size=1, stride=1, dilation=1):
        super(Bottleneck_wo_norm, self).__init__()
        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=1, bias=False)
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=kernel_size, stride=stride, dilation=dilation, padding=dilation - 1, bias=False)
        self.conv3 = nn.Conv3d(planes, planes * self.expansion, kernel_size=1, bias=False)
...
class CoordConv_Block(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=1, stride=1, padding=0, dilation=1, groups=1):
        super(CoordConv_Block, self).__init__()
        self.coordBlock_1 = nn.Sequential(
            nn.Conv3d(in_planes + 3, out_planes, kernel_size=kernel_size, stride=stride, padding=padding,
                      dilation=dilation, groups=groups, bias=True),
            nn.InstanceNorm3d(out_planes, affine=True),
            nn.ReLU(inplace=True),
            nn.Conv3d(out_planes, in_planes, kernel_size=kernel_size, stride=stride, padding=padding,
                      dilation=dilation, groups=groups, bias=True),
            nn.InstanceNorm3d(out_planes, affine=True),
        )

    def forward(self, x, coord):
        x = F.relu(x + self.coordBlock_1(torch.cat([x, coord], dim=1)), inplace=True)
        return x

class CoordConv_Block_wo_norm(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=1, stride=1, padding=0, dilation=1, groups=1):
        super(CoordConv_Block_wo_norm, self).__init__()
        self.coordBlock_1 = nn.Sequential(
            nn.Conv3d(in_planes + 3, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv3d(out_planes, in_planes, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=True),
        )
...
class AddCoords_size(nn.Module):
    ...
    def forward(self, size):
        ...
        if self.with_r:
            rr = torch.sqrt(torch.pow(xx_channel - 0.5, 2) + torch.pow(yy_channel - 0.5, 2) + torch.pow(zz_channel - 0.5, 2)).unsqueeze(0)
...
class Multi_Heads_Self_Attn_1D(nn.Module):
    def __init__(self, in_plane, out_plane, n_heads = 4, d_k = 16):
        ...
        self.output_conv = nn.Conv1d(self.num_heads * self.d_k, out_plane, kernel_size=1, bias=True)
        self.act_f = nn.ReLU(inplace=True)
...
class Multi_Heads_Self_Attn_Q_KV(nn.Module):
    def __init__(self, n_featuremap_q, n_featuremap_kv, n_heads = 4, d_k = 16):
        ...
        """ query """
        self.query_conv = nn.Conv3d(n_featuremap_q, self.num_heads * self.d_k, kernel_size=1, padding=0, bias=True)
        self.key_conv = nn.Conv3d(n_featuremap_kv, self.num_heads * self.d_k, kernel_size=1, padding=0, bias=True)
        self.value_conv = nn.Conv3d(n_featuremap_kv, self.num_heads * self.d_k, kernel_size=1, bias=True)

        self.softmax = nn.Softmax(dim=-1)
        self.drop = nn.Dropout(p=0.1)
        self.output_conv = nn.Conv3d(self.num_heads * self.d_k, n_featuremap_kv, kernel_size=1, bias=True)
        self.norm_layer = nn.InstanceNorm3d(n_featuremap_kv)
        self.act_f = nn.LeakyReLU(inplace=True)
    ...
    def forward(self, x_q, x_kv):
        ...
        weights = self.drop(torch.sigmoid(logits))  # BX (N) X (N/p)
...
class Attention_gate(nn.Module):
    ...
    def forward(self, x_l, x_g):
        ...
        phi_g = F.interpolate(self.phi(x_g), size=theta_x_size[-3:], mode='trilinear', align_corners=False)
...
class MAB(nn.Module):
    ...
    def forward(self, Q, K):
        ...
        """ query scale"""
        query_scale = np.power(self.dim_v, -0.5)
        logits = torch.matmul(queries * query_scale, keys.permute(0,1,3,2))
        weights = self.softmax(logits)  # BX (N) X (N/p)
        out = torch.matmul(weights, values) + queries # B, n_h, N, f
        # out = torch.matmul(weights, values)
        ...
        """ merge heads """
        out = self._merge_heads(out)
        out = out.view(m_batchsize, spatial_dim_Q, -1).permute(0, 2, 1)
        out = out + self.act_f(self.output_conv(out))
        # out = self.act_f(self.output_conv(out))
...
class GradCam:
    ...
    def __call__(self, x):
        image_size = (x.size(-1), x.size(-2), x.size(-3))
        heat_maps = []
